...
const Footer = () => {
  ...
  const siteDescription = config?.site_description || 'Tu destino para el estilo y la moda. Calidad premium, diseño innovador.';
  const siteLogo = config?.site_logo;
  const instagramUrl = config?.instagram_url || 'https://www.instagram.com/modawearshop2_alterna?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==';
  const whatsappNumber = config?.whatsapp_number || '';
  const whatsappUrl = whatsappNumber ? `https://wa.me/${whatsappNumber.replace(/[^0-9]/g, '')}` : 'https://wa.link/l8vc70';
  // Mostrar WhatsApp siempre, usar URL por defecto si no hay número configurado
  
  // Debug: Log cuando cambia el config
  useEffect(() => {
    console.log('Footer - Config changed:', config);
  }, [config]);
  return (
